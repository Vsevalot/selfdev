...
from simplemail import Message, SimpleMailException
...
def save_to_metadata(file_name: str, emails: List[Message]) -> None:
    with open(file_name, 'w') as file:
        for email in emails:
            text = METADATA_TEMPLATE.format(email.get_date(), email.get_id())
            file.write(f"{text}\n")

def save_to_metadata_old(file_name: str, emails: List[Message]) -> None:
    with open(file_name, 'w') as file:
        counter = 1
        for email in emails:
            text = METADATA_TEMPLATE_OLD.format(counter,
                                                email.get_id(),
                                                email.get_date(),
                                                email.get_sender(),
